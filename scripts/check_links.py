...
"""
opcgate.com 链接健康度检查
- 扫 data/policies.json 所有政策的 url（application.url + links.official + links.news）
- HEAD + GET 双重核验（内容关键词）
- 输出 data/link-health.json（线上可消费）
- 可选参数 --fail-on-dead N：死链超过 N 条则 exit(1)（给 deploy.sh 用）
    ...
    if not is_binary:
        try:
            ...
            with urllib.request.urlopen(req, timeout=TIMEOUT, context=ctx) as r:
                ...
                # 空壳/JS-only 页面（例如 szsiq/beijing 首页 JS 路由）
                if len(content) < 200:
...
def main():
    ...
    print(f"🔍 开始检测 {len(tasks)} 条链接（来自 {len(policies)} 条政策）...")
